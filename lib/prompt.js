...
prompt.get = function (schema, callback) {
  // Used to "tag" debugging messages
  var tag = Math.floor(Math.random()*100);

  console.error('%s: prompt.get(%j, cb)'.magenta, tag, schema);

  iterate(schema, function get(target, next) {
    var path = target.path,
        schema = target.schema;
    //console.error('%s: prompt.getInput(%j, cb)'.cyan, tag, target);
  ...
  function iterate(schema, get, cb) {
    ...
    console.log('%s: iterator: %j'.blue, tag, iterator);
    ...
    async.forEachSeries(iterator, function (branch, next) {
      get(branch, function assembler(err, line) {
        ...
        function attach(path, line) {
          result[path.shift()] = build(path, line);
        }
...
prompt.getInput = function (prop, callback) {
  var tag = Math.floor(Math.random()*100);
  console.error('%d: getInput(%s, cb)'.yellow, tag, util.inspect(prop));

  // TODO: clean up all this to work correctly. :|
  var schema = prop.schema || prop,
      propName = prop.path && prop.path.join(':') || prop,
      storedSchema = prompt.properties[propName.toLowerCase()],
      name = (prop.description) || propName,
      delim = prompt.delimiter,
      raw = [prompt.message, delim + name.grey, delim.grey],
      read = (prop.hidden
        ? prompt.readLineHidden
        : prompt.readLine
      ),
      defaultLine, length, msg, valid, against;
  ...
  console.log('%d: %s'.magenta, tag, util.inspect({
    schema: schema,
    propName: propName,
    name: name
  }));
  ...
  if (prompt.override && prompt.override[propName]) {
    //console.error('prompt.override: callback(null, %j);', prompt.override[propName]);
  ...
  read.call(null, function (err, line) {
    ...
    console.log('%d: validate(%s, %s) == %j;'.cyan, tag,
      util.inspect(against), util.inspect(schema), valid);
    ...
    //console.error('callback(null, %j);', line);
    callback(null, line);
  });
};
