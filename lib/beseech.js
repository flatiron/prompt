...
var beseech = module.exports = Object.create(events.EventEmitter.prototype);
...
beseech.started    = false;
beseech.paused     = false;
beseech.allowEmpty = false; 
...
// #### @options {Object} **Optional** Options to consume by beseech
...
//
beseech.start = function (options) {
  if (beseech.started) {
    return;
  }
  ...
  beseech.allowEmpty = options.allowEmpty || false;
...
beseech.pause = function () {
  if (!beseech.started || beseech.paused) {
    return;
  }
  ...
  read.call(null, function (err, line) {
    ...
    if (prop.validator) {
      ...
      if (!valid) {
        ...
        beseech.emit('invalid', prop, line);
